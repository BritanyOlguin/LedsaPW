...
use App\Models\ImgValores;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Storage;

class ImgValoresController extends Controller
{
    public function bannerData()
    {
        $datosBanner = ImgValores::all();
        return response()->json($datosBanner);
    }

    public function bannerDataNew()
    {
        $datosBanner = ImgValores::oldest()->take(1)->get();
        return response()->json($datosBanner);
    }
    ...
    public function editarBanner(Request $request)
    {
        ...
        $banner = ImgValores::find($request->id);
