...
use App\Models\ImgVision;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Storage;

class ImgVisionController extends Controller
{
    public function bannerData()
    {
        $datosBanner = ImgVision::all();
        return response()->json($datosBanner);
    }

    public function bannerDataNew()
    {
        $datosBanner = ImgVision::oldest()->take(1)->get();
        return response()->json($datosBanner);
    }
    ...
    public function editarBanner(Request $request)
    {
        ...
        $banner = ImgVision::find($request->id);
